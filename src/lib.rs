...
impl<T: Eq + Clone> RleVec<T> {
    ...
    pub fn set_range(&mut self, mut start: usize, len: usize, value: T) {
        if len == 0 { return; }
        if len == 1 { return self.set(start, value); }
    ...
    fn set_range_internal(&mut self, start: usize, start_run_idx: usize, end: usize, end_run_idx: usize, value: T) {
...
mod tests {
    ...
    fn assert_postconditions<T: Eq>(rle: &RleVec<T>) {
        assert!(rle.runs[0].end < rle.len());
        for i in 1..rle.runs.len() {
            assert!(rle.runs[i].end < rle.len());
